...
function listIssues(filters, user) {
  ...
  // 列表加载时也刷新过期状态，避免已过期项目仍显示为进行中
  visibleRows.forEach(issue => {
    const refreshed = refreshDeadline(issue);
    if (refreshed !== issue) Object.assign(issue, refreshed);
  });
...
function getIssueDetail(id, user) {
  ...
  const refreshed = refreshDeadline(issue);
  const currentIssue = refreshed !== issue ? refreshed : repo.getIssueById(id);
...
function refreshDeadline(issue) {
  if (!issue.deadline || ['已完成', '未完成', '已终止'].includes(issue.status)) return issue;
  if (issue.deadline >= new Date().toISOString().slice(0, 10)) return issue;
  const updated = repo.advanceStatus(issue.id, '已终止', '结束时间已到，项目自动终止');
