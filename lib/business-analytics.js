...
function getMergedDishSalesAnalytics(db, params = {}) {
  ...
  const finish = list => list.map(item => {
    const totalCost = Math.round((item.total_cost || 0) * 100) / 100;
    ...
    if (out.sources_map) {
      out.sources = [...out.sources_map.values()].map(s => ({
        ...s,
        quantity: round3(s.quantity), sales_amount: round2(s.sales_amount),
        income_amount: round2(s.income_amount), cost: round2(s.cost),
      })).sort((a, b) => b.quantity - a.quantity);
      out.platform_count = out.platforms ? out.platforms.size : out.sources.length;
    }
    if (out.sku_map) {
      out.sku_breakdown = [...out.sku_map.values()].map(s => ({
