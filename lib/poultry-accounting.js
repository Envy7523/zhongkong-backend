...
// 单条保存（门店+日期+禽类 唯一，重复保存覆盖）。批量保存复用它，避免两套校验漂移。
function saveConsumption(body = {}) {
  ...
  return writeConsumption({ storeId, storeName: store.store_name, date, birdId, quantity, remark: body.remark });
}
function writeConsumption({ storeId, storeName, date, birdId, quantity, remark }) {
  ...
  if (existing) {
    db.run(`UPDATE poultry_consumption SET quantity=?, remark=?, store_name=?,
            updated_at=datetime('now','localtime') WHERE id=?`,
      [quantity, String(remark || '').trim(), storeName, existing.id]);
  } else {
    db.insert(
      'INSERT INTO poultry_consumption (store_id,store_name,date,bird_id,quantity,remark) VALUES (?,?,?,?,?,?)',
      [storeId, storeName, date, birdId, quantity, String(remark || '').trim()]);
  }
  return existing ? 'updated' : 'created';
}
// 批量保存：一次提交同一「门店+日期」下的多个禽类，界面就不用一天录三次。
// 每条独立校验，单条失败不影响其它条；返回逐条结果便于界面提示。
function saveConsumptionBatch(body = {}) {
  const storeId = Number(body.store_id);
  if (!Number.isInteger(storeId) || storeId <= 0) throw new Error('请选择门店');
  const date = String(body.date || '').trim();
  if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) throw new Error('日期格式应为 YYYY-MM-DD');
  const entries = Array.isArray(body.entries) ? body.entries : [];
  if (!entries.length) throw new Error('请至少填写一个禽类的消耗只数');
  const store = db.queryOne('SELECT id, store_name FROM stores WHERE id=?', [storeId]);
  if (!store) throw new Error('门店不存在');
  const results = [];
  let created = 0; let updated = 0; let skipped = 0;
  entries.forEach(entry => {
    const birdId = Number(entry?.bird_id);
    const raw = entry?.quantity;
    // 留空=这次不录这个禽类，跳过（不清零也不报错）
    if (raw === '' || raw === null || raw === undefined) { skipped += 1; return; }
    const quantity = Number(raw);
    if (!Number.isInteger(birdId) || birdId <= 0) { results.push({ bird_id: entry?.bird_id, error: '禽类品种不存在' }); return; }
    if (!Number.isFinite(quantity) || quantity < 0) { results.push({ bird_id: birdId, error: '消耗只数不能为负' }); return; }
    const bird = db.queryOne('SELECT id, animal, breed_name FROM poultry_birds WHERE id=?', [birdId]);
    if (!bird) { results.push({ bird_id: birdId, error: '禽类品种不存在' }); return; }
    try {
      const action = writeConsumption({ storeId, storeName: store.store_name, date, birdId, quantity, remark: body.remark });
      if (action === 'created') created += 1; else updated += 1;
      results.push({ bird_id: birdId, label: `${bird.animal} · ${bird.breed_name}`, quantity, action });
    } catch (error) {
      results.push({ bird_id: birdId, error: error.message });
    }
  });
  if (!created && !updated) throw new Error('没有可保存的消耗只数（请至少填一个禽类）');
  db.save();
  return { ok: true, date, store_id: storeId, created, updated, skipped, failed: results.filter(item => item.error).length, results };
}
...
module.exports = {
  ...
  listConsumption, saveConsumption, saveConsumptionBatch, deleteConsumption,
