...
/**
 * 采购校准：把「模型理论只数」与「门店录入的实际只数」按月对比，算偏差与 MAPE。
 *
 * 为什么需要它：评价参数好坏不该靠「腿占下庄 15%~80%」这类经验阈值，而该看
 * **历史数据拟合**——改完系数后如果滚动 MAPE 从 7.8% 降到 2.6%，才算真的改善。
 * 数据来源：`poultry_purchases`（门店实际消耗/采购只数，手动录入，UNIQUE(store_id, month, bird_id)）。
 *
...
function calibrationReport(params = {}) {
  ...
  return {
    ...
    notes: skippedInvalid
      ? `已跳过 ${skippedInvalid} 条无效记录（所属禽类已被删除的历史遗留采购行，无法参与校准）`
      : '',
  };
}
...
module.exports = {
  ...
  calculate, purchaseComparison, calibrationReport,
