...
function boolInt(v) {
  // 导入器可上报 imported=<本次导入的行数>；任何有限正数均表示已导入。
  // 不能只接受数字 1，否则批次 191 的 imported=140 会被错误记录为否。
  if (v === true || v === 'true') return 1;
  if (typeof v === 'number' && Number.isFinite(v)) return v > 0 ? 1 : 0;
  if (typeof v === 'string' && /^\d+$/.test(v)) return Number(v) > 0 ? 1 : 0;
  return 0;
}
...
function overview(db) {
  // 「今日成功/失败」是运行事件发生的北京时间日期，不是报表的业务日期。
  ...
  const todayCounts = db.queryOne(
    `SELECT
     ...
     FROM sync_job_runs WHERE date(COALESCE(last_event_at, created_at), '+8 hours')=?`, [today]);
