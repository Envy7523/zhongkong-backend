...
function createCoverageHandler({ db, audit, activeStoresProvider, logger } = {}) {
  ...
  return function coverageHandler(req, res) {
    ...
    let count = 0;
    try {
      const row = db.queryOne('SELECT COUNT(*) AS n FROM business_revenue_records WHERE biz_date=?', [businessDate]);
...
module.exports = { PATH, MAX_ACTIVE_STORES, ACTIVE_STORES_BASIS, isRealDate, loopbackOnly, rawGetBody, verifyRequest, createStoresStatusActiveProvider, readActiveStores, createCoverageHandler, mountCoverage };
