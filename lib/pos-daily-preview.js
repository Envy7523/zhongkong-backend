...
function buildPosDailyPreview(db, businessDate, options = {}) {
  ...
  for (const row of rows) {
    ...
    // 收入构成包含“外卖”付款项（图片已在渠道栏展开，不重复展示），因此与全部营业收入核对。
    const compositionTotal = roundMoney(Object.values(row.compositions).reduce((n, value) => n + value, 0));
    if (Object.keys(row.compositions).length && Math.abs(compositionTotal - row.recorded_amount) > 0.02)
      problems.push(`收入构成与营业收入不平：${row.store_name}`);
  ...
  if (rows.length < 9) problems.push('高位、中位、低位需至少 9 家门店，当前不足以生成互不重叠的分组');
  const middle = Math.floor((rows.length - 3) / 2);
  const focusGroups = [
    { key: 'top', rows: rows.slice(0, 3).map(row => ({ ...row, rank_label: `TOP${row.rank}` })) },
    { key: 'middle', rows: rows.slice(middle, middle + 3).map(row => ({ ...row, rank_label: String(row.rank) })) },
    { key: 'bottom', rows: rows.slice(-3).map((row, index) => ({ ...row, rank_label: `最差${3 - index}` })) },
  ];
  const focusRows = focusGroups.flatMap(group => group.rows);
  const totals = {
  ...
  return {
    ...
    store_count: rows.length, excluded_stores: excluded, rows, focus_rows: focusRows, focus_groups: focusGroups, totals,
