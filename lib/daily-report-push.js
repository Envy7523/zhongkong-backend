...
async function pushAllStoreDailyReports(ctx) {
  const { cfg, httpPost, ensureDailyDataLoaded, db, now, business_date: businessDate } = ctx;
  ...
  // 自动日报必须证明数据来源日期与请求日期相同；当前共享日报缓存没有可信日期，故拒发。
  const sourceDate = ctx.dailyStoreState && ctx.dailyStoreState.business_date;
  if (!/^\d{4}-\d{2}-\d{2}$/.test(String(businessDate || '')) || sourceDate !== businessDate) {
    return { ok: false, sent: false, reason: 'daily_source_date_unverified' };
  }
