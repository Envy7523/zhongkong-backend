...
// 前缀 -> 权限码。顺序：更具体的写在前面的位置不重要，匹配时按前缀长度取最长。
const PERMISSION_RULES = [
  ...
  ['/api/db/stores', 'store.manage'],
  ['/api/stores', 'store.manage'],
  ['/api/store-regions', 'store.manage'],
  ['/api/store-platforms', 'store.manage'],
  ['/api/store-models', 'store.manage'],
...
/** 取某条路径需要的权限码；返回 null 表示"登录即可" */
function requiredPermission(pathname) {
  const path = String(pathname || '');
  if (ADMIN_ONLY.some(prefix => prefixMatches(path, prefix))) return '*';
  let best = null;
  let bestLength = -1;
  for (const [prefix, code] of PERMISSION_RULES) {
    if (prefixMatches(path, prefix) && prefix.length > bestLength) { best = code; bestLength = prefix.length; }
  }
  return best;
}
...
function isPublicPath(pathname) {
  const path = String(pathname || '');
  return ALWAYS_ALLOWED.some(prefix => prefixMatches(path, prefix));
}
