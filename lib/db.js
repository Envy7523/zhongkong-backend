...
function createTables() {
  ...
  db.run(`INSERT OR IGNORE INTO push_profiles (code,name,description,content_template,image_template,enabled)
    VALUES ('group_buy_daily','团购每日总结日报','数据分析 · 团购视角 · 每日总结专用日报；目标机器人由统一消息路由显式分配。','## {{title}}\n> 日期：{{date}}　|　已汇报门店：**{{store_count}}** 家\n\n{{body}}','',1)`);
  // 新版推送注册表：凭证只在写入时接收，列表只返回掩码；每种消息显式绑定一个机器人。
  db.run(`CREATE TABLE IF NOT EXISTS wecom_webhook_bots (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    name TEXT NOT NULL UNIQUE,
  ...
  try { db.run("ALTER TABLE wecom_webhook_bots ADD COLUMN source TEXT NOT NULL DEFAULT 'manual'"); } catch {}
  db.run(`CREATE TABLE IF NOT EXISTS wecom_message_routes (
    message_code TEXT PRIMARY KEY,
    bot_id INTEGER REFERENCES wecom_webhook_bots(id) ON DELETE SET NULL,
    enabled INTEGER NOT NULL DEFAULT 0,
    revision INTEGER NOT NULL DEFAULT 0,
    updated_at TEXT DEFAULT (datetime('now','localtime'))
  )`);
  try { db.run('ALTER TABLE wecom_message_routes ADD COLUMN revision INTEGER NOT NULL DEFAULT 0'); } catch {}
  db.run("INSERT OR IGNORE INTO wecom_message_routes(message_code,bot_id,enabled) VALUES ('pos_daily',NULL,0)");
  db.run("INSERT OR IGNORE INTO wecom_message_routes(message_code,bot_id,enabled) VALUES ('group_buy_daily',NULL,0)");
  db.run(`CREATE TABLE IF NOT EXISTS pos_daily_test_sends (
  ...
  db.run(`CREATE TABLE IF NOT EXISTS pos_daily_scope_exclusions (
    store_id INTEGER PRIMARY KEY REFERENCES stores(id) ON DELETE CASCADE,
