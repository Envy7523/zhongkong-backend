...
function syncDefaultWebhookBot(db, config = {}) {
  ...
  if (!validWebhook(webhook)) {
    if (row && (row.enabled || row.webhook_url)) {
      db.run("UPDATE wecom_message_routes SET enabled=0,revision=revision+1,updated_at=datetime('now','localtime') WHERE bot_id=?", [row.id]);
...
function createRouter({ db, loadConfig, saveConfig }) {
  ...
  const publicRoute = row => ({ message_code: row.message_code, name: MESSAGE_CODES[row.message_code] || row.message_code,
    bot_id: row.bot_id, bot_name: row.bot_name || '', enabled: Boolean(row.enabled), ready: Boolean(row.enabled && row.bot_id && row.bot_enabled), revision: Number(row.revision || 0) });

  router.get('/', (_req, res) => {
    try {
      const bots = db.queryAll('SELECT id,name,enabled,webhook_url,updated_at FROM wecom_webhook_bots ORDER BY id').map(publicBot);
      const routes = db.queryAll('SELECT r.message_code,r.bot_id,r.enabled,r.revision,b.name AS bot_name,b.enabled AS bot_enabled FROM wecom_message_routes r LEFT JOIN wecom_webhook_bots b ON b.id=r.bot_id ORDER BY r.message_code').map(publicRoute);
  ...
  router.put('/bots/:id', (req, res) => {
    try {
      ...
      if (prior.source === 'default_config' && (name !== prior.name || webhook !== prior.webhook_url)) {
        ...
        db.run("UPDATE wecom_message_routes SET enabled=0,revision=revision+1,updated_at=datetime('now','localtime') WHERE bot_id=?", [id]);
        db.save();
  ...
  router.put('/routes/:code', (req, res) => {
    try {
      ...
      db.run("UPDATE wecom_message_routes SET bot_id=?,enabled=0,revision=revision+1,updated_at=datetime('now','localtime') WHERE message_code=?", [bot?.id || null, code]);
      db.save();
  ...
  router.post('/routes/:code/activation', (req, res) => {
    try {
      ...
      const row = db.queryOne('SELECT r.bot_id,r.revision,b.enabled AS bot_enabled FROM wecom_message_routes r LEFT JOIN wecom_webhook_bots b ON b.id=r.bot_id WHERE r.message_code=?', [code]);
      if (!row) return res.status(404).json({ error: '消息路由不存在' });
      if (enabled && (!row.bot_id || Number(row.bot_enabled) !== 1)) return res.status(400).json({ error: '请先为该项目消息分配可用机器人' });
      if (enabled && (Number(req.body?.expected_revision) !== Number(row.revision) || Number(req.body?.expected_bot_id) !== Number(row.bot_id)))
        return res.status(409).json({ error: '机器人分配或凭证已变化，请刷新页面并重新核对目标群与日报样式' });
