...
const { pushAllStoreDailyReports } = require('./daily-report-push');
...
function mountSchedule({ app, express, db, permissions, syncJobAudit, daily, logger }) {
  ...
  /* ---------------- 项目侧日报推送（复用既有逻辑，不发明新渲染） ---------------- */
  async function pushDailyReports({ business_date }) {
    let cfg = null;
    try { cfg = daily.loadConfig(); } catch (e) { return { ok: false, sent: false, reason: 'config_unavailable' }; }
    return pushAllStoreDailyReports({
      cfg, business_date,
      httpPost: daily.httpPost,
      ensureDailyDataLoaded: daily.ensureDailyDataLoaded,
      dailyStoreState: daily.dailyStoreState,
      drawStoreDailyReport: daily.drawStoreDailyReport,
      toReportData: daily.toReportData,
      crypto: daily.crypto,
      db,
    });
  }
