    ...
    <section class="import-layout">
      <main class="upload-panel">
        ...
        <div v-if="result" :class="['import-result', result.type]">
          ...
          <div><b>{{ result.title }}</b><p>{{ result.message }}</p></div>
        </div>
...
const configs = {
  ...
  'group-buy': {
    ...
    platforms: ['美团团购', '抖音团购'],
    groups: [
      { title: '基础识别', fields: [{ name: '日期', required: true, note: '平台结算对应的营业日期' }, { name: '门店ID / 门店名称', required: true, note: '至少填写一项' }, { name: '渠道', required: true, note: '美团团购或抖音团购' }] },
...
async function submitImport() {
  ...
  try {
    ...
    result.value = { type: 'success', title: '导入完成', message: `写入 ${response.imported} 条营收记录${response.product_rows ? `、${response.product_rows} 条商品销量` : ''}${response.skipped ? `；跳过 ${response.skipped} 行异常数据` : ''}。` }
    file.value = null
  } catch (error) {
