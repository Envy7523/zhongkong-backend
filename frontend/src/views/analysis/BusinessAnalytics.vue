<template>
  <div class="business-analytics-page" :class="{ 'compare-mode': compareThemeActive }">
    ...
    <section class="filter-bar">
      ...
      <el-date-picker v-if="filters.timeMode === 'day'" v-model="filters.day" type="date" value-format="YYYY-MM-DD" placeholder="选择日期" />
      <el-date-picker v-else-if="filters.timeMode === 'week'" v-model="filters.week" type="week" value-format="YYYY-MM-DD" format="YYYY 第 ww 周" placeholder="选择周次" />
      <el-date-picker v-else-if="filters.timeMode === 'month'" v-model="filters.month" type="month" value-format="YYYY-MM" placeholder="选择月份" />
      <el-date-picker v-else v-model="filters.customRange" type="daterange" value-format="YYYY-MM-DD" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" />
      <el-select v-if="analysisMode === 'store'" v-model="filters.storeId" filterable placeholder="选择门店" style="width:220px"><el-option v-for="store in storeOptions" :key="store.id" :label="store.store_name" :value="store.id" /></el-select>
      <el-select v-if="analysisMode === 'custom'" v-model="filters.storeIds" multiple collapse-tags collapse-tags-tooltip filterable placeholder="选择汇总门店" style="width:300px"><el-option v-for="store in storeOptions" :key="store.id" :label="store.store_name" :value="store.id" /></el-select>
    ...
    <template v-if="activeTab === 'overview'">
      ...
      <template v-else>
        <section class="chart-grid"><article class="panel"><header><div><h3>{{ analysisMode === 'store' ? '门店实收走势图' : '实收趋势' }}</h3><p>按{{ trendTimeModeLabel }}口径聚合</p></div><span class="panel-badge">{{ data.trend.length }} 个周期</span></header><div v-if="data.trend.length" ref="trendChartRef" class="chart"></div><el-empty v-else :description="emptyDescription" /></article><article class="panel"><header><div><h3>渠道营业构成</h3><p>{{ scopeChip }}渠道构成</p></div></header><div v-if="channelCompositionItems.length" ref="compositionChartRef" class="chart"></div><el-empty v-else description="暂无构成数据" /></article></section>
      </template>
    ...
    <el-dialog v-model="compareDialogVisible" title="设置环比时间" width="620px" class="compare-dialog" destroy-on-close @closed="handleCompareDialogClosed">
      <div class="compare-form"><div><h4>当前时间</h4><p>{{ compareCurrentLocked ? '当前页面已有查询结果，当前时间已锁定。' : '当前页面尚未查询，可在此编辑当前时间。' }}</p><el-date-picker v-if="filters.timeMode === 'day'" v-model="compareCurrentSelection" type="date" value-format="YYYY-MM-DD" :disabled="compareCurrentLocked" @change="updateCompareCurrent" /><el-date-picker v-else-if="filters.timeMode === 'week'" v-model="compareCurrentSelection" type="week" value-format="YYYY-MM-DD" format="YYYY 第 ww 周" :disabled="compareCurrentLocked" @change="updateCompareCurrent" /><el-date-picker v-else-if="filters.timeMode === 'month'" v-model="compareCurrentSelection" type="month" value-format="YYYY-MM" :disabled="compareCurrentLocked" @change="updateCompareCurrent" /><el-date-picker v-else v-model="compareCurrentSelection" type="daterange" value-format="YYYY-MM-DD" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" :disabled="compareCurrentLocked" @change="updateCompareCurrent" /></div><div><h4>环比时间</h4><p>{{ compareTimeHint }}</p><el-date-picker v-if="filters.timeMode === 'day'" v-model="comparePreviousSelection" type="date" value-format="YYYY-MM-DD" @change="updateComparePrevious" /><el-date-picker v-else-if="filters.timeMode === 'week'" v-model="comparePreviousSelection" type="week" value-format="YYYY-MM-DD" format="YYYY 第 ww 周" @change="updateComparePrevious" /><el-date-picker v-else-if="filters.timeMode === 'month'" v-model="comparePreviousSelection" type="month" value-format="YYYY-MM" @change="updateComparePrevious" /><el-date-picker v-else v-model="comparePreviousSelection" type="daterange" value-format="YYYY-MM-DD" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" @change="updateComparePrevious" /></div></div><template #footer><el-button @click="compareDialogVisible = false">取消</el-button><el-button type="primary" :disabled="!compareRangesReady" @click="applyCompareMode">查询环比</el-button></template>
    </el-dialog>
...
import { generateBusinessDiagnosis, getBusinessAnalytics, getBusinessDiagnosis, getDishSalesAnalytics, getStores } from '@/api'
...
const analysisScope = computed(() => route.meta.analysisScope || 'total')
const analysisMode = computed(() => route.meta.analysisMode || 'brand')
const routePlatform = computed(() => route.meta.analysisPlatform || '')
...
const viewConfig = computed(() => ({ title: route.meta.analysisTitle || '总数据视角', description: analysisScope.value === 'total' ? '从日、周、月和自定义时间四种口径，查看品牌、单店或自选门店的经营汇总。' : `${analysisScope.value === 'group-buy' ? '团购' : '外卖'}数据支持平台、品牌与门店统计；门店可切换平台分别查看或平台汇总。` }))
...
const diagnosisParams = computed(() => {
  ...
  return { period: aggregationPeriod.value, store_id: analysisMode.value === 'store' ? filters.storeId || undefined : undefined, store_ids: analysisMode.value === 'custom' ? filters.storeIds.join(',') || undefined : undefined, channel_group: scopeChannelGroup.value || undefined, platform: effectivePlatform.value || undefined, date_from: range?.[0] || '', date_to: range?.[1] || '' }
})
const diagnosisSignature = computed(() => JSON.stringify({ perspective_key: route.meta.analysisKey || route.path, ...diagnosisParams.value }))
...
function analyticsParams(range, storeId) { return { period: aggregationPeriod.value, trend_period: analysisMode.value === 'store' ? 'day' : undefined, store_id: storeId || (analysisMode.value === 'store' ? filters.storeId || undefined : undefined), store_ids: storeId ? undefined : (analysisMode.value === 'custom' ? filters.storeIds.join(',') : undefined), channel_group: scopeChannelGroup.value || undefined, platform: effectivePlatform.value || undefined, reconciliation_page: reconciliationPage.value, reconciliation_page_size: reconciliationPageSize.value, date_from: range?.[0], date_to: range?.[1] } }
...
async function refreshDiagnosis() {
  ...
  try {
    const result = await getBusinessDiagnosis({ perspective_key: route.meta.analysisKey || route.path, filter_signature: signature })
...
async function runDiagnosis() {
  ...
  try {
    const result = await generateBusinessDiagnosis({ perspective_key: route.meta.analysisKey || route.path, filter_signature: diagnosisSignature.value, perspective_title: viewConfig.value.title, analysis_params: diagnosisParams.value })
    diagnosis.value = result.diagnosis || null
...
async function openStoreDetail(store) {
  ...
  try {
    const result = await getBusinessAnalytics('overview', { period: aggregationPeriod.value, store_id: store.store_id, channel_group: scopeChannelGroup.value || undefined, platform: effectivePlatform.value || undefined, date_from: range[0], date_to: range[1] })
...
function renderCharts() {
  ...
  if (trendChartRef.value) {
    ...
    trendChart.resize()
    trendChart.setOption({ color: isStoreTrend ? ['#3b82f6'] : series.map(item => item.color), tooltip: { trigger: 'axis', valueFormatter: value => money(value) }, legend: { show: isStoreTrend, top: 2, right: 6, itemWidth: 10, itemHeight: 6, textStyle: { color: '#667085' } }, grid: { left: 12, right: 12, top: 42, bottom: 5, containLabel: true }, xAxis: { type: 'category', data: data.trend.map(item => item.period), axisLine: { lineStyle: { color: '#e4e9f1' } }, axisTick: { show: false }, axisLabel: { color: '#8a94a6' } }, yAxis: { type: 'value', axisLabel: { color: '#8a94a6', formatter: value => value >= 10000 ? `${(value / 10000).toFixed(1)}万` : value }, splitLine: { lineStyle: { color: '#edf1f6', type: 'dashed' } } }, series: isStoreTrend ? [{ name: '实收', type: 'line', smooth: true, data: data.trend.map(row => row.total || 0), symbol: 'circle', symbolSize: 7, lineStyle: { width: 3 }, itemStyle: { color: '#3b82f6' }, areaStyle: { color: 'rgba(59,130,246,.16)' } }] : series.map((item, index) => ({ name: item.name, type: 'bar', stack: series.length > 1 ? 'revenue' : undefined, data: data.trend.map(row => row[item.key] || 0), barMaxWidth: 26, itemStyle: { borderRadius: index === series.length - 1 ? [4, 4, 0, 0] : [0, 0, 4, 4] } })) }, true)
  }
