...
$key = 'F:\NewDeom\server-access\zhongkong_company'
...
Say '== 3/7 上传后端 =='
scp -q -i $key "F:\NewDeom\server.js" "${hostS}:$APP/server.js"; Say ("   server.js exit=$LASTEXITCODE")
scp -q -i $key "F:\NewDeom\lib\db.js" "${hostS}:$APP/lib/db.js"; Say ("   lib/db.js exit=$LASTEXITCODE")
scp -q -i $key "F:\NewDeom\lib\poultry-accounting.js" "${hostS}:$APP/lib/poultry-accounting.js"; Say ("   lib/poultry-accounting.js (新增) exit=$LASTEXITCODE")
scp -q -i $key "F:\NewDeom\lib\business-analytics.js" "${hostS}:$APP/lib/business-analytics.js"; Say ("   lib/business-analytics.js exit=$LASTEXITCODE")
scp -q -i $key "F:\NewDeom\PROJECT_CHANGES.md" "${hostS}:$APP/PROJECT_CHANGES.md"; Say ("   PROJECT_CHANGES.md exit=$LASTEXITCODE")

Say '== 4/7 上传前端（先清旧 dist，避免残留旧哈希分包） =='
Run "cd $APP/frontend && rm -rf dist && echo DIST_CLEARED"
scp -q -r -i $key "F:\NewDeom\frontend\dist" "${hostS}:$APP/frontend/"; Say ("   dist exit=$LASTEXITCODE")
scp -q -r -i $key "F:\NewDeom\frontend\src" "${hostS}:$APP/frontend/"; Say ("   src exit=$LASTEXITCODE")
...
Say '== 7/7 校验数据库新表（只读） =='
scp -q -i $key "F:\NewDeom\_tmp\check-cloud-tables.js" "${hostS}:$APP/_check_table.js"
...
Say '===== 部署结束 ====='
$log -join "`n" | Out-File -FilePath 'F:\NewDeom\_deploy_poultry_log.txt' -Encoding utf8
